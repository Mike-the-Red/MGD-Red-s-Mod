...
label advanceTime(TimeIncrease):
    $ HealingSickness -= 1
    $ renpy.log("Advancing time by "+str(TimeIncrease)+".")
    
    python:
      try:
        persistant.timeLock
      except:
        persistant.timeLock = False
    
    if(persistant.timeLock):
      $ renpy.log("Stop fucking around with time, you shithead!")
      return
    else:
      $ persistant.timeLock = True
    
    while TimeIncrease >= 1:
        $ renpy.log("  Time advancing.")
        ...
        # call TimeEvent(CardType="TimePassed", LoopedList=TimePassedList) from _call_TimeEvent_7 # Red's Note: This event is hugely bugged. It seems to be causing infinite loops all the time.
...
label returnToTown:
    ...
    # $ renpy.log("Return to town advancing time.")
...
label PurgeFetishes:
    ...
    $ c = 0
    while c < len(TempFetishes):
        if c < len(TempFetishes):
            if TempFetishes[c].Type == "Fetish":
                $ theTotalCost += TempFetishes[c].Level*costPerFetish
        $ c += 1


    while i < len(TempFetishes):
        if i < len(TempFetishes):
            if TempFetishes[i].Type == "Fetish":
                $ theCost += TempFetishes[i].Level*costPerFetish

                $ display = TempFetishes[i].name + ": " + str(TempFetishes[i].Level*costPerFetish) + " eros."

                if display1 == "" and TempFetishes[i].Level*costPerFetish > 0:
                    $ display1 = display
                    $ d1Name = TempFetishes[i].name
                    $ d1Cost = TempFetishes[i].Level*costPerFetish
                elif display2 == "" and TempFetishes[i].Level*costPerFetish > 0:
                    $ display2 = display
                    $ d2Name = TempFetishes[i].name
                    $ d2Cost = TempFetishes[i].Level*costPerFetish
                elif display3 == "" and TempFetishes[i].Level*costPerFetish > 0:
                    $ display3 = display
                    $ d3Name = TempFetishes[i].name
                    $ d3Cost = TempFetishes[i].Level*costPerFetish
                elif display4 == "" and TempFetishes[i].Level*costPerFetish > 0:
                    $ display4 = display
                    $ d4Name = TempFetishes[i].name
                    $ d4Cost = TempFetishes[i].Level*costPerFetish
        $ i += 1
    ...
    if theCost > 0:
        ...
        menu fetPicker:
            "[display1]" if display1 != "":
                if player.inventory.money >= d1Cost:
                    $ theCost = d1Cost
                    $ player.FetishList[getFromName(d1Name,player.FetishList)].Level -= TempFetishes[getFromName(d1Name,TempFetishes)].Level
                    $ TempFetishes[getFromName(d1Name,TempFetishes)].Level = 0
                    $ fetCount = 0
                    jump payFet
                else:
                    jump lackOfMoneyGoddess
            "[display2]" if display2 != "":
                if player.inventory.money >= d2Cost:
                    $ theCost = d2Cost
                    $ player.FetishList[getFromName(d2Name,player.FetishList)].Level -= TempFetishes[getFromName(d2Name,TempFetishes)].Level
                    $ TempFetishes[getFromName(d2Name,TempFetishes)].Level = 0
                    $ fetCount = 0
                    jump payFet
                else:
                    jump lackOfMoneyGoddess
            "[display3]" if display3 != "":
                if player.inventory.money >= d3Cost:
                    $ theCost = d3Cost
                    $ player.FetishList[getFromName(d3Name,player.FetishList)].Level -= TempFetishes[getFromName(d3Name,TempFetishes)].Level
                    $ TempFetishes[getFromName(d3Name,TempFetishes)].Level = 0
                    $ fetCount = 0
                    jump payFet
                else:
                    jump lackOfMoneyGoddess
            "[display4]" if display4 != "":
                if player.inventory.money >= d4Cost:
                    $ theCost = d4Cost
                    $ player.FetishList[getFromName(d4Name,player.FetishList)].Level -= TempFetishes[getFromName(d4Name,TempFetishes)].Level
                    $ TempFetishes[getFromName(d4Name,TempFetishes)].Level = 0
                    $ fetCount = 0
                    jump payFet
                else:
                    jump lackOfMoneyGoddess
