    ...
    def SceneRequiresCheck():
        ...
        while checkPreFuncs == 0:
            if displayingScene.theScene[lineOfScene] == "HideOptionOnRequirementFail" and hideFailedMenuChoice == 0:
                ...
                lineOfScene += 1
            # ***** Red's Additions *****
            elif displayingScene.theScene[lineOfScene] == "RequiresProgressFromEvent":
              passProgressChecks += 1
              lineOfScene += 1
              eventName = displayingScene.theScene[lineOfScene]
              lineOfScene += 1
              target = int(displayingScene.theScene[lineOfScene])

              eventIndex = getFromName(eventName, ProgressEvent)
              progress = ProgressEvent[eventIndex].eventProgress
              renpy.log("  Event \""+ eventName +"\" progress is "+ str(progress) +", it should be "+ str(target) +":")
              if progress == target:
                # renpy.log("    Passed.")
                passProgressCheck += 1
              # else:
                # renpy.log("    Failed.")
              lineOfScene += 1
            elif displayingScene.theScene[lineOfScene] == "RequiresPartner":
              lineOfScene += 1
              tempVarName = displayingScene.theScene[lineOfScene]
              renpy.log("  Partner is \""+ adventurePartner.name +"\" and should be \""+ tempVarName +"\".")
              if(tempVarName == "None"):
                tempVarName = ""
              if(adventurePartner.name != tempVarName):
                # renpy.log("    Failed.")
                varCheck = False
              # else:
                # renpy.log("    Passed.")
              lineOfScene += 1
            elif displayingScene.theScene[lineOfScene] == "RequiresVar":
              lineOfScene += 1
              tempVarName = displayingScene.theScene[lineOfScene]
              tempVar = globals().get(tempVarName)
              renpy.log("  Variable \"" + tempVarName + "\" is " + str(tempVar) + ":")
              if(not tempVar):
                # renpy.log("    Failed.")
                varCheck = False
              # else:
                # renpy.log("    Passed.")
              lineOfScene += 1
            elif displayingScene.theScene[lineOfScene] == "RequiresVarValue":
              # Red's Note: Only works with global variables. Attributes of a global class won't work. I may fix this at some point.
              lineOfScene += 1
              tempVarName = displayingScene.theScene[lineOfScene]
              lineOfScene += 1
              tempVarValue = displayingScene.theScene[lineOfScene]
              tempVar = globals().get(tempVarName)
              renpy.log("  Variable \"" + tempVarName + "\" is " + str(tempVar) + " " + str(type(tempVar)) + " and should be " + str(tempVarValue) + ".")
              if(str(tempVar) != str(tempVarValue)):
                # renpy.log("    Failed.")
                varCheck = False
              # else:
                # renpy.log("    Passed.")
        ...
        # renpy.log("  End of SceneRequiresCheck. Current Line: " + displayingScene.theScene[lineOfScene])
        # renpy.log("  End of SceneRequiresCheck. Previous Line: " + displayingScene.theScene[lineOfScene-1])
        # renpy.log("  Scene Requirements: " + str(passcheck) + " & " + str(varCheck))
        return (passcheck and varCheck)
...
label resumeSceneAfterCombat:
    $ renpy.log("======================")
    $ renpy.log("=   Resuming Scene   =")
    $ renpy.log("======================")
    $ renpy.log("Scene Name: " + displayingScene.NameOfScene)
    $ showOnSide = 0
    $ showSpeakers = 1
    $ display1 = ""
    $ display2 = ""
    ...
    # $ lastLine = -1
    while(lineOfScene < len(displayingScene.theScene)): #  and lastLine != lineOfScene
        $ readLine = 0
        $ notFunction = 0
        # $ lastLine = lineOfScene # Red's Addition: Added to break infinite loop. But, it didn't work.
        ...
        if displayingScene.theScene[lineOfScene] == "PlayerSpeaks":
            ...
            $ lineOfScene += 1
            $ DataLocation = getFromName(ProgressEvent[DataLocation].name, ProgressEvent) # Red's Note: Does this just set it to itself?
            ...
            if displayingScene.theScene[lineOfScene] == "DelayNotifications":
                ...
                $ lineOfScene -= 1
            # $ renpy.log("Scene resting player.")
            ...
            # $ renpy.log("Scene sleeping player.")
            ...
            if hasThing == 1:
                ...
                $ lineOfScene += 1
        elif displayingScene.theScene[lineOfScene] == "IfHasItems":
            ...
            if passSkillCheck == 1:
                ...
                if len(monsterEncounter) > 0:
                    return
        elif displayingScene.theScene[lineOfScene] == "IfHasSkills":
            $ hasThing = 0
            $ ThingNeeded = 0
            python:
                while displayingScene.theScene[lineOfScene] != "EndLoop":
          ...
          # ----- Red's New Code -----
          $ renpy.pause(delay=0.01, hard=True) # Stop skipping
          python:
            global menuIndex, menuLines, displayableMenu, _skipping
            menuLines = []
            tempLine = {}
            currentMenuItem = -1
            overrideLines = []
            ...
            # Gather Choices
            # renpy.log("~~~ Gathering Menu Choices ~~~")
            lineOfScene += 1
            while(lineOfScene < len(displayingScene.theScene) and displayingScene.theScene[lineOfScene] != "EndLoop"):
              # renpy.log("Menu Line: " + displayingScene.theScene[lineOfScene])
              if(displayingScene.theScene[lineOfScene] == "OverrideOption"):
                lineOfScene += 1
                # renpy.log("  Overriding Choice: " + displayingScene.theScene[lineOfScene])
                if(displayingScene.theScene[lineOfScene] == "FinalOption"):
                  overrideLines.append("FinalOption")
                elif(displayingScene.theScene[lineOfScene] not in overrideLines):
                  overrideLines.append(displayingScene.theScene[lineOfScene])
              elif(displayingScene.theScene[lineOfScene] in requirementChecks):
                # renpy.log("  Scene Req Check: " + displayingScene.theScene[lineOfScene])
                # renpy.log(menuLines[currentMenuItem])
                # menuLines[currentMenuItem]["valid"] &= SceneRequiresCheck() # This was used when I thought requirements went after the modified line!
                reqs &= SceneRequiresCheck()
                lineOfScene -= 1 # Going back one line, because requirements check advances too far here.
                # renpy.log("  Requirements for next menu item are "+str(reqs)+".")
              elif(displayingScene.theScene[lineOfScene] == "MaxMenuSlots"):
                lineOfScene += 1
                # renpy.log("Menu Slots Set: " + displayingScene.theScene[lineOfScene])
                MaxMenuSlots = int(displayingScene.theScene[lineOfScene])
              elif(displayingScene.theScene[lineOfScene] == "Caption"):
                lineOfScene += 1
                # renpy.log("Menu Caption: " + displayingScene.theScene[lineOfScene])
                caption = displayingScene.theScene[lineOfScene]
              elif(displayingScene.theScene[lineOfScene] == "ChoiceJump"):
                lineOfScene += 1
                # renpy.log("Choice \""+ menuLines[currentMenuItem]["text"] +"\" Jump Location: "+ displayingScene.theScene[lineOfScene])
                menuLines[currentMenuItem]["jump"] = displayingScene.theScene[lineOfScene]
              else:
                # renpy.log("  Adding Choice: " + displayingScene.theScene[lineOfScene])
                menuLines.append({"text":displayingScene.theScene[lineOfScene], "jump":displayingScene.theScene[lineOfScene], "valid":reqs})
                reqs = True
                ++currentMenuItem
              lineOfScene += 1
            
            # Build Displayable Menu
            # renpy.log("~~~ Building Displayable Menu ~~~")
            if(ShuffleMenuOptions):
              random.shuffle(menuLines)
            if(overrideLines):
              for over in range(len(overrideLines)):
                # renpy.log("Overriding Search: " + overrideLines[over])
                if(overrideLines[over] == "FinalOption" and len(menuLines)>1):
                  menuLines.remove(len(menuLines)-1)
                else:
                  for choice in range(len(menuLines)):
                    if(overrideLines[over] == menuLines[choice]):
                      # renpy.log("  Removing Overridden Choice: " + choice["text"])
                      overrideLines.remove(over)
                      menuLines.remove(choice)
                      break
            
            # # renpy.log("Menu Lines:")
            # # renpy.log(menuLines)
            for choiceNum in range(len(menuLines)):
              # # renpy.log("  Choice " + str(choiceNum))
              if(choiceNum > MaxMenuSlots * (1 + displayIndex)): # Menu pages
                # renpy.log("Adding Menu Page: " + str(displayIndex))
                ++displayIndex
              if(menuLines[choiceNum]["valid"]):
                tempLine = {
                              "page":displayIndex,
                              "text":menuLines[choiceNum]["text"],
                              "jump":menuLines[choiceNum]["jump"]
                            }
                # # renpy.log("Adding Display Choice " + str(tempLine.__dict__))
                displayableMenu.append(tempLine)
            
            # renpy.log("~~~ Showing Menu ~~~")
            # Show Menu Screen
            # renpy.log("Displayable Menu:")
            # renpy.log(displayableMenu)
            if(displayIndex > 0):
              renpy.show_screen("redsMenuPageButtons", displayIndex)
            renpy.show_screen("redsMenu", displayableMenu, caption)
            renpy.pause()
            
            SceneBookMarkRead = 0
            lineOfScene = 0
            # renpy.log("Next Scene Name: " + displayingScene.NameOfScene)
            # _skipping = couldSkip
            renpy.jump("resumeSceneAfterCombat")
          
          # ----- Old code with a bug somewhere (in recheck?) -----
            # $ index = 0
            # $ lineOfScene += 1
...
###########################################Assorted functions####################################################
        ...
        # ***** Red's Additions *****
        elif displayingScene.theScene[lineOfScene] == "IfVarContinue":
          $ lineOfScene += 1
          $ tempVarName = displayingScene.theScene[lineOfScene]
          $ tempVar = globals().get(tempVarName)
          if(not tempVar):
            $ renpy.log("Variable \"" + tempVarName + "\" is " + str(tempVar) + "; jumping to alternate scene.")
            $ lineOfScene += 1
            $ display = displayingScene.theScene[lineOfScene]
            call sortMenuD from _call_sortMenuD_8000
            if len(monsterEncounter) > 0:
              return
          else:
            $ renpy.log("Variable \"" + tempVarName + "\" is " + str(tempVar) + "; continuing scene.")
        elif displayingScene.theScene[lineOfScene] == "IfVarValueContinue":
          $ lineOfScene += 1
          $ tempVarName = displayingScene.theScene[lineOfScene]
          $ lineOfScene += 1
          $ tempVarValue = displayingScene.theScene[lineOfScene]
          $ tempVar = globals().get(tempVarName)
          if(str(tempVar) != str(tempValue)):
            $ renpy.log("Variable \"" + tempVarName + "\" is " + str(tempVar) + " " + type(tempVar) + ", not " + str(tempVarValue) + "; jumping to alternate scene.")
            $ lineOfScene += 1
            $ display = displayingScene.theScene[lineOfScene]
            call sortMenuD from _call_sortMenuD_8001
            if len(monsterEncounter) > 0:
              return
          else:
            $ renpy.log("Variable \"" + tempVarName + "\" is " + str(tempVar) + "; continuing scene.")
        ...
        elif displayingScene.theScene[lineOfScene] == "SetVarBool":
          $ lineOfScene += 1
          $ tempVarName = displayingScene.theScene[lineOfScene]
          $ lineOfScene += 1
          $ tempValue = bool(displayingScene.theScene[lineOfScene])
          # $ tempVar = globals().get(tempVarName)
          if(len(tempVarName) > 0):
            $ renpy.log("Updating variable \"" + tempVarName + "\" to \"" + str(tempValue) + "\".")
            $ globals().update([[tempVarName, tempValue]])
        elif displayingScene.theScene[lineOfScene] == "SetVarInt":
          $ lineOfScene += 1
          $ tempVarName = displayingScene.theScene[lineOfScene]
          $ lineOfScene += 1
          $ tempValue = int(displayingScene.theScene[lineOfScene])
          # $ tempVar = globals().get(tempVarName)
          if(len(tempVarName) > 0):
            $ renpy.log("Updating variable \"" + tempVarName + "\" to \"" + str(tempValue) + "\".")
            $ globals().update([[tempVarName, tempValue]])
        elif displayingScene.theScene[lineOfScene] == "SetVarFloat":
          $ lineOfScene += 1
          $ tempVarName = displayingScene.theScene[lineOfScene]
          $ lineOfScene += 1
          $ tempValue = float(displayingScene.theScene[lineOfScene])
          # $ tempVar = globals().get(tempVarName)
          if(len(tempVarName) > 0):
            $ renpy.log("Updating variable \"" + tempVarName + "\" to \"" + str(tempValue) + "\".")
            $ globals().update([[tempVarName, tempValue]])
        elif displayingScene.theScene[lineOfScene] == "SetVarString":
          $ lineOfScene += 1
          $ tempVarName = displayingScene.theScene[lineOfScene]
          $ lineOfScene += 1
          $ tempValue = displayingScene.theScene[lineOfScene]
          # $ tempVar = globals().get(tempVarName)
          if(len(tempVarName) > 0):
            $ renpy.log("Updating variable \"" + tempVarName + "\" to \"" + str(tempValue) + "\".")
            $ globals().update([[tempVarName, tempValue]])
        
        elif displayingScene.theScene[lineOfScene] == "SetPartner":
          if(PartnerDatabase == []): # Ensure that the database is loaded! 
            $ loadPartnerDatabase()
          
          $ lineOfScene += 1
          $ partnerName = displayingScene.theScene[lineOfScene]
          if(len(partnerName) > 0):
            $ renpy.log("Scene changing partner to " + partnerName + ".")
            $ adventurePartner.setDataByID(partnerName)
          else:
            $ renpy.log("Scene removing partner.")
            $ canPartner = True
            $ adventurePartner = AdventurePartner()
        elif displayingScene.theScene[lineOfScene] == "CallLabel":
          $ lineOfScene += 1
          $ renpy.call(displayingScene.theScene[lineOfScene])
        elif displayingScene.theScene[lineOfScene] == "Eval":
          $ lineOfScene += 1
          $ renpy.eval(displayingScene.theScene[lineOfScene])
        elif displayingScene.theScene[lineOfScene] == "Wait":
          $ lineOfScene += 1
          $ renpy.pause(float(displayingScene.theScene[lineOfScene]))
        elif displayingScene.theScene[lineOfScene] == "Dissolve":
          python:
            dur = 0.5
            try: dur = float(displayingScene.theScene[lineOfScene +1])
            except: pass
            else: lineOfScene += 1
          with Dissolve(dur)
        elif displayingScene.theScene[lineOfScene] == "Log":
          $ lineOfScene += 1
          $ renpy.log(str(lineOfScene) +": "+ displayingScene.theScene[lineOfScene])
        elif displayingScene.theScene[lineOfScene] == "ShowHealth":
        ...
        # ***** End Red's Additions *****
        ...
        elif displayingScene.theScene[lineOfScene] == "StopSoundEffect":
            stop sound fadeout 1.0
        elif displayingScene.theScene[lineOfScene] == "StopSoundEffect2":
            ...
            if persistent.showVFX == True:
                show dark onlayer visualEffects at BlackOut with Dissolve(0.5) # Red's Note: Added transition
        elif displayingScene.theScene[lineOfScene] == "EndBlackOut":
            hide dark onlayer visualEffects with Dissolve(0.5) # Red's Note: Added transition
            ...
            if check == 1:
                ...
                "[display]"
            # $ renpy.log("--- Giving Perk ---")
            # $ renpy.log(monsterEncounter)
            # $ renpy.log(str(displayingScene.theScene))
...
label TownLocation:
    ...
    $ timeNotify = 0
    # $ renpy.log("Town location advancing time by zero.")
