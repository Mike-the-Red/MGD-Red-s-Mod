...
init python:
        ...
        for perk in target.perks:
            ...
            while  p < len(perk.PerkType):
                if perk.PerkType[p] == "SleepAmp":
                    SleepAmp += (perk.EffectPower[p]*-1)*0.01
                p += 1
    ...
    def getTheEndScene(Scenes, Monster, monsterEncounter, trueMonsterEncounter, lastAttack):
        ...
        sceneNumbers  = []
        howFar = 0
        renpy.log("True Monster Encounter")
        renpy.log(trueMonsterEncounter)
        ...
        for each in Scenes:
            ...
            foundMatch = 0
            inUse = []
            renpy.log("  Each of Scenes")
            renpy.log("  "+ each.NameOfScene +" | "+ each.picture +" | "+ str(each.includes) +" | "+ each.move +" | "+ each.stance)
            if len(each.includes) > 1:
                if len(monsterEncounter) > 1:
                    ...
                    for monReq in each.includes:
                        ...
                        for monsAvalible in monsterEncounter: # Red's change
        ...
        renpy.log("Scene Numbers")
        renpy.log(sceneNumbers)
        renpy.log("ScenesPool")
        renpy.log(scenesPool)
        if len(scenesPool) == 1:
            chosenScene = sceneNumbers[0]
            renpy.log("End Chosen Scene: #"+str(chosenScene)+" - "+Scenes[chosenScene].NameOfScene)
        elif(len(scenesPool) > 0):# Red's bug fix
          if chosenScene == -5:
              renpy.log("  First group.")
          ...
          if chosenScene == -5:
              renpy.log("Last scene check.")
              chosenScene = sceneNumbers[renpy.random.randint(0, max(0, len(scenesPool)-1))] # Red's bug fix
        else:
          # bugScene = {}
          # bugScene.NameOfScene = "Bug Found"
          # bugScene.move = ""
          # bugScene.stance = ""
          # bugScene.includes = []
          # bugScene.picture = ""
          # bugScene.theScene = ["Log", "Error: Scene called when no scene exists from \"getTheEndScene\".",]
          # return bugScene
          raise Exception("No end scene found.")
...
label combat:
    ...
    if(adventurePartner.name != ""):
      $ adventurePartner.combatPartnerImage()
    if(persistant.duplicateEnemies):
      python:
        initialEnemyRange = range(len(monsterEncounter))
        for i in initialEnemyRange:
          if(monsterEncounter[i].generic):
            monsterEncounter.append(copy.deepcopy(monsterEncounter[i]))
...
label enemySkillChoice(mSC):
    ...
    python:
        ...
        if largestScene > 1:
            clearList = 0
            while clearList < len(refinedSkillList): 
                if len(refinedSkillList[clearList].requiresTargetStance) < largestScene:
...
label combatDisplay:
    ...
    while playerGone == 0 or lastMonToGo < 0:#who acts first
        ...
        if playerInititive >= playerCheck and playerGone == 0:
            ...
            # Adventure Partner Addition
            if(adventurePartner.name != ""):
              $ renpy.log("Partner combat turn.")
              $ partnerChoice = adventurePartner.selectTarget(player, monsterEncounter)
              if(len(partnerChoice) > 0 and partnerChoice[0] is not None):
                # python:
                #   for i in range(len(partnerChoice[1])):
                #     renpy.log("Partner plans to use \""+partnerChoice[0].name+"\" against \""+str(partnerChoice[1][i])+"\".")
                $ i = 0
                $ targs = len(partnerChoice[1])
                $ skillChoice = partnerChoice[0]
                $ nameExtra = partnerChoice[0].name
                $ Speaker = Character(_(adventurePartner.name + " - " + nameExtra))
                $ attacker = adventurePartner
                while(i < targs):
                  if(partnerChoice[1][i] == -1):
                    $ renpy.log("Partner uses \""+partnerChoice[0].name+"\" on herself.")
                    $ defender = adventurePartner
                    $ CombatFunctionEnemytarget = m # -2
                    $ CombatFunctionEnemyInitial = m # -2
                  elif(partnerChoice[1][i] == -2):
                    $ renpy.log("Partner uses \""+partnerChoice[0].name+"\" on you.")
                    $ defender = player
                    $ CombatFunctionEnemytarget = m # -1
                    $ CombatFunctionEnemyInitial = m # -1
                  else:
                    $ renpy.log("Partner uses \""+partnerChoice[0].name+"\" against \""+monsterEncounter[partnerChoice[1][i]].name+"\".")
                    $ defender = monsterEncounter[partnerChoice[1][i]]
                    $ CombatFunctionEnemytarget = partnerChoice[1][i]
                    $ CombatFunctionEnemyInitial = partnerChoice[1][i]
                  $ i += 1
                  call combatActionTurn # Use this one instead, once I have the friendly targeting figured out.
                  $ attacker = player # Changing it back to the player. Hopefully this will help to avoid some wonky win/lose scenes.
            if len(monsterEncounter) == 0: #combat is over
              jump combatWin
            ...
            for perk in player.perks:
                ...
                while  p < len(perk.PerkType):
                    if perk.PerkType[p] == "RegenMaxArousal":
                        hpRegen += perk.EffectPower[p]
                    if perk.PerkType[p] == "RegenMaxEnergy":
                        epRegen += perk.EffectPower[p]
                    p += 1
            if hpRegen != 0:
                player.stats.hp -= int(math.floor(player.stats.max_true_hp*((hpRegen)*0.01)))
            if epRegen != 0:
                player.stats.ep += int(math.floor(player.stats.max_true_ep*((epRegen)*0.01)))
...
label combatLoss:
    if(attacker.name == adventurePartner.name):
      $ attacker = player
    if(defender.name == adventurePartner.name):
      $ defender = player
    $ hidingCombatEncounter = 0
    $ VicChosenScene = -5
    $ victoryScene = 0
    ...
    elif(theLastAttacker.name == adventurePartner.name): # Red's tweak
      $ renpy.log("Partner was last attacker, changing to first monster.")
      $ theLastAttacker = monsterEncounter[0]
      $ renpy.log(theLastAttacker)
      $ renpy.log(monsterEncounter)
      $ renpy.log(trueMonsterEncounter)
      $ renpy.log(lastAttack)
...
label combatLossEnd:
    ...
    $ player.stats.refresh()
    $ timeNotify = 1
    # $ renpy.log("Combat loss advancing time.")
    call advanceTime(TimeIncrease=1) from _call_advanceTime_6
    $ timeNotify = 1
    while(TimeOfDay != "Morning"): # Red's Bug Fix
      if(TimeOfDay != "Morning"):
        $ timeNotify = 1
        # $ renpy.log("Combat loss advancing time until morning.")
...
label combatWin:
    ...
    # Red's Addition
    if(adventurePartner.name != ""):
      $ adventurePartner.casualPartnerImage()
...
label PostCombatWin:
    ...
    if noDoubleRewards == 0:
        ...
        if MoneyDrops > 0:
            $ bonusMoney = 100
            python:
                for perk in player.perks: # Red's tweak
                    p = 0
                    while  p < len(perk.PerkType):
