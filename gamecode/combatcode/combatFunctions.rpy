            ...
            while atkLo < loop:
                ...
                if(skillChoice.skillType == "attack"):
                    python:
                        ...
                        # Red's Addition
                        if(attacker.generic and adventurePartner.name != ""):
                          holder[0] = int(holder[0] * persistant.genericDamageMult)
                        elif(attacker.name != player.name and attacker.name != adventurePartner.name and adventurePartner.name != ""):
                          holder[0] = int(holder[0] * persistant.uniqueDamageMult)
                        ...
                        if skillChoice.statusEffect == "Analyze":
                            ...
                            python:
                                ...
                                if  len(StrongArray) > 0:
                                    ri = 0
                                    for each in StrongArray:
                                        if defender.resistancesStatusEffects.getRes(each) < 150:
                                            resDoubleChek=1
                                            display += each
                                            if ri + 1 < len(StrongArray):
                                                display += ", "
...
label combatFunctions:
    python:
        ...
        def DefeatMonster( monLossCheck):
            ...
            lvlDifference = 1
            if player.stats.lvl > monsterEncounter[monLossCheck].stats.lvl:
                lvlDifference += (monsterEncounter[monLossCheck].stats.lvl*5 - player.stats.lvl*5)*0.01 + (player.stats.lvl)*0.01
                if lvlDifference <= 0.7:
                    lvlDifference = 0.7
                if lvlDifference > 1:
                    lvlDifference = 1

            monsExp = monsterEncounter[monLossCheck].stats.Exp*lvlDifference
            ExpBoost = 1
            ...
            DefeatedEncounterMonsters.append(copy.deepcopy(monsterEncounter[monLossCheck])) # Red's tweak
            ...
            # del trueMonsterEncounter[monLossCheck] # Red's Tweak
